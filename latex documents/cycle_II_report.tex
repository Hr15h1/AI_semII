...
\usepackage[colorlinks=true, linkcolor=black, linktoc=all, pdfstartview=FitH]{hyperref}
\usepackage{fancyhdr}
\usepackage{geometry}
\usepackage{lipsum}
...
\begin{center}
  \textbf{\fontsize{15}{15}\selectfont SECOND SEMESTER M.Sc COMPUTER SCIENCE WITH}
  \vspace{1em}
  \textbf{\fontsize{15}{15}\selectfont SPECIALIZATION IN ARTIFICIAL INTELLIGENCE}
  \vspace{6em}\\
  \textbf{\large (2024 - 2026 BATCH)}
  \vspace{1em}\\
  \textbf{\large LAB RECORD}
  \vspace{1em}\\
  \textbf{\large CAI-CC-524 PATTERN RECOGNITION LABORATORY}
  \vspace{6em}\\
  \textbf{\textit{\small Submitted to the University of Kerala in partial fulfilment of the requirement for the award of the degree of Master of Science in Computer Science (with Specialization in Artificial Intelligence)}}
  \vspace{3em}\\

  \begin{center}
    \includegraphics[width=0.27\textwidth]{University_of_Kerala.png}
  \end{center}

  \vspace{3em}
  \textbf{\textit{\small Submitted By}}
  \vspace{1em}\\
  \textbf{\fontsize{15}{15}\selectfont Hrishikesh M Jayakrishnan}
  \vspace{1em}\\
  \textbf{\fontsize{15}{15}\selectfont 97424607010}
  \vspace{3em}\\
  \textbf{\textit{\small Department of Computer Science,}}
  \vspace{1em}\\
  \textbf{\textit{\small University of Kerala, Kariavattom,}}
  \vspace{1em}\\
  \textbf{\textit{\small Thiruvananthapuram, Kerala, 695581}}

\end{center}

\newpage
\thispagestyle{empty}
\section*{\underline{Index}}
\begin{longtable}{|c|p{11cm}|c|} 
\hline
\textbf{Sl No} & \textbf{Name of the program} & \textbf{Page No} \\
\hline
1 & LDA for dimensionality reduction & \pageref{exp1} \\
\hline
2 & PCA for dimensionality reduction and KNN & \pageref{exp2} \\
\hline
3 & PCA on Iris dataset & \pageref{exp3} \\
\hline
4 & Euclidean distance between two grayscale images & \pageref{exp4} \\
\hline
5 & PCA vs LDA on MNIST dataset & \pageref{exp5} \\
\hline
6 & Implementing distance metrics on image dataset & \pageref{exp6} \\ 
\hline
7 & Operations on fuzzy sets and De Morgan's Law & \pageref{exp7} \\
\hline
8 & Membership functions using NumPy & \pageref{exp8} \\
\hline
9 & Fuzzification of input temperature values & \pageref{exp9} \\
\hline
10 & Comparison of different defuzzification methods & \pageref{exp10} \\
\hline
11 & Calculate value functions for a given Markov Decision Process & \pageref{exp11} \\
\hline
\end{longtable}
\newpage

\pagenumbering{arabic}
\section*{\underline{Cycle II}}
\begin{center}
...
\vspace{1em}
\noindent
\textbf{\underline{Aim and Objectives}}\par
\noindent
...
\vspace{1em}
\noindent
\textbf{\underline{Comments on comparisons}}
\noindent
\begin{itemize}
...
\textbf{\underline{Problem}}\par
\noindent
Write a Python program to plot Triangular, Trapezoidal, and Bell-shaped membership functions using NumPy and Matplotlib
...
\textbf{\underline{Aim and Objectives}}\par
\noindent
To understand the different types of membership functions in fuzzy logic, specifically Triangular, Trapezoidal, and Bell-shaped membership functions, and to visualize them using NumPy and Matplotlib.
\vspace{1em}
\noindent
\textbf{\underline{Program Code}}
\noindent
...
\begin{lstlisting}[language=Python]
y = [0.1, 0.5, 0.9, 0.5, 0.1]
plt.plot(set_a, y, 'r', label = 'Fuzzy Set A')
\end{lstlisting}

\begin{center}
  \includegraphics[width=0.65\textwidth]{output_II_54.png}
\end{center}
...
\begin{lstlisting}[language=Python]
y = [0.1, 0.5, 0.5, 0.5, 0.1]
plt.plot(set_a, y, 'b', label = 'Fuzzy Set B')
\end{lstlisting}

\begin{center}
  \includegraphics[width=0.65\textwidth]{output_II_55.png}
\end{center}
...
\begin{lstlisting}[language=Python]
set_a = np.linspace(0, 10, 100)

width = 2
slope = 2.5
center = 5

m_a = 1 / (1 + np.abs((set_a - center) / width) ** slope)

plt.plot(set_a, m_a, 'g', label = 'Bell-shaped Membership Function')
plt.xlabel('Universe of Discourse')
plt.ylabel('Membership Degree')
\end{lstlisting}

\begin{center}
  \includegraphics[width=0.65\textwidth]{output_II_56.png}
\end{center}
...
def triangular(x, a, b, c):
    """Triangular membership function with feet at a and c, peak at b."""
    return np.maximum(np.minimum((x - a) / (b - a), (c - x) / (c - b)), 0)

def trapezoidal(x, a, b, c, d):
    """Trapezoidal membership function with slopes from a to b and c to d."""
    return np.maximum(np.minimum(np.minimum((x - a)/(b - a), 1), (d - x)/(d - c)), 0)

def bell_shaped(x, a, b, c):
    """Generalized bell-shaped membership function centered at c."""
...
tri_y = triangular(x, 2, 5, 8)
trap_y = trapezoidal(x, 2, 4, 6, 8)
bell_y = bell_shaped(x, a=1, b=4, c=5)

# --- Plotting ---
plt.figure(figsize=(10, 6))

plt.plot(x, tri_y, label="Triangular (a=2, b=5, c=8)", color='blue')
plt.plot(x, trap_y, label="Trapezoidal (a=2, b=4, c=6, d=8)", color='green')
plt.plot(x, bell_y, label="Bell-shaped (a=1, b=4, c=5)", color='red')

plt.title("Triangular, Trapezoidal, and Bell-Shaped Membership Functions")
plt.xlabel("x")
plt.ylabel("Membership Degree")
plt.ylim(-0.1, 1.1)
plt.grid(True)
...
\noindent
\begin{itemize}
    \item The AI-generated code defines functions for triangular, trapezoidal, and bell-shaped membership functions, while my code directly plots the membership functions without defining separate functions.
    \item The AI code uses a more generalized approach to define the parameters of the membership functions, allowing for easy adjustments, while my code uses fixed parameters.
    \item Both codes use NumPy and Matplotlib for calculations and plotting.
\end{itemize}
\vspace{1em}
\vfill
\noindent
...
\textbf{\underline{Aim and Objectives}}\par
\noindent
To fuzzify the input variable temperature using Triangular membership functions for Low, Medium, and High categories with specified input temperature values.
\vspace{1em}
\noindent
\textbf{\underline{Program Code}}
\noindent
...
for temp in temperature:
    chance\_low = fuzzy.interp\_membership(temps, low, temp)
    chance\_medium = fuzzy.interp\_membership(temps, medium, temp)
    chance\_high = fuzzy.interp\_membership(temps, high, temp)
    ...
    max\_chance = max(chances, key=chances.get)

    print(f"Temperature {temp} C: is a {max\_chance}")
\end{lstlisting}
...
\noindent
\begin{itemize}
  \item The AI-generated code uses the `skfuzzy` library to create triangular membership functions for Low, Medium, and High temperature categories, similar to my code.
  \item Both codes define the temperature range and fuzzify the input variable using triangular membership functions.
  \item The AI code uses `interp\_membership` to calculate the membership degree for each temperature value and displays the probabilities for a temperature to be in all three categories. 
\end{itemize}
\vspace{1em}
\noindent
...
\begin{center}
  \includegraphics[width=0.65\textwidth]{output_II_58.png}
\end{center}
...
\textbf{\underline{Aim and Objectives}}\par
\noindent
To understand and implement defuzzification methods using a given fuzzy output set, and to visualize the fuzzy output while highlighting each defuzzified result on the plot.
\vspace{1em}
\noindent
\textbf{\underline{Program Code}}
\noindent
...
\noindent
\begin{itemize}
    \item The AI-generated code uses the `skfuzzy` library to implement defuzzification methods, similar to my code.
    \item Both codes define a fuzzy output set and visualize the defuzzification methods on a plot.
\end{itemize}
\vspace{1em}
\noindent
\textbf{\underline{Results}}
...
\begin{center}
  \includegraphics[width=0.65\textwidth]{output_II_59.png}
\end{center}
...
\textbf{\underline{Aim and Objectives}}\par
\noindent
To understand and implement Markov Decision Processes (MDPs) using a transition matrix and reward function, and to calculate the value function for each state given a policy initialized as 0.5.
...
\begin{lstlisting}[language=Python]
def policy_evaluation(states, actions, policy, transitions, discount_factor, theta=1e-6):
...
\begin{lstlisting}[language=Python]
v = policy_evaluation(states, actions, policy, transitions, discount_factor)
\end{lstlisting}
...
\noindent
\begin{itemize}
    \item The AI-generated code uses NumPy to define the transition matrix and reward function, similar to my code.
    \item Both codes implement a policy evaluation function to calculate the value function for each state.
    \item The AI code structures the transition matrix and reward function in a more compact way, while my code uses dictionaries for better readability.
\end{itemize}
\vspace{1em}
\noindent
\textbf{\underline{Results}}
